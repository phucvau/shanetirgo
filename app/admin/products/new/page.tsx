...
export default function NewProductPage() {
  ...
  const colorBreakdown = useMemo(() => {
    const grouped = new Map<string, { sizes: Set<string>; total: number }>();
    variants.forEach((item) => {
      if (!grouped.has(item.color)) {
        grouped.set(item.color, { sizes: new Set<string>(), total: 0 });
      }
      const bucket = grouped.get(item.color);
      if (!bucket) return;
      bucket.sizes.add(item.size);
      bucket.total += item.stock;
    });
    return Array.from(grouped.entries()).map(([color, value]) => ({
      color,
      sizes: Array.from(value.sizes).join(", "),
      total: value.total,
    }));
  }, [variants]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (uploadedImageUrls.length === 0) {
        throw new Error("Không thể upload ảnh sản phẩm.");
      }
      ...
      if (!createResponse.ok) {
        throw new Error(createResult?.message || "Tạo sản phẩm thất bại.");
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="font-serif text-2xl font-bold text-foreground">Tạo sản phẩm mới</h1>
        <p className="text-sm text-muted-foreground">Nhập theo ma trận màu-size-tồn kho để dễ quản lý biến thể quần áo.</p>
      </div>
          ...
          <CardContent className="space-y-5">
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <Input placeholder="Thêm màu (VD: Đen)" value={newColor} onChange={(e) => setNewColor(e.target.value)} />
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <Input placeholder="Size tùy chỉnh (VD: 3XL)" value={customSize} onChange={(e) => setCustomSize(e.target.value)} />
            ...
            <p className="text-xs text-muted-foreground">
              Để trống ô nếu biến thể không tồn tại. Nhập <strong>0</strong> nếu biến thể có nhưng đã hết hàng.
            </p>

            <div className="space-y-2 rounded-md border bg-muted/40 p-3 text-sm">
              {colorBreakdown.length === 0 ? (
                <p className="text-muted-foreground">Chưa có biến thể nào được nhập tồn kho.</p>
              ) : (
                colorBreakdown.map((row) => (
                  <div key={row.color} className="grid gap-2 rounded-md border bg-background px-3 py-2 md:grid-cols-3">
                    <p><span className="text-muted-foreground">Màu:</span> {row.color}</p>
                    <p><span className="text-muted-foreground">Size:</span> {row.sizes || "-"}</p>
                    <p><span className="text-muted-foreground">Tồn kho:</span> {row.total}</p>
                  </div>
                ))
              )}
              <p className="pt-1"><span className="text-muted-foreground">Tổng tồn toàn sản phẩm:</span> {totalStock}</p>
              <p><span className="text-muted-foreground">Tổng size đã dùng:</span> {sizeSummary || "-"}</p>
              <p><span className="text-muted-foreground">Tổng màu đã dùng:</span> {colorSummary || "-"}</p>
            </div>
          </CardContent>
        </Card>
      ) : (
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <Input
              ...
              placeholder="Nhập tồn kho cho phụ kiện"
            />
            <p className="text-xs text-muted-foreground">Phụ kiện không bắt buộc size và màu.</p>
          </CardContent>
        </Card>
      )}
